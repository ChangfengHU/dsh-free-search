  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        keyStorageCredHint: (c) => `保存后写入 ~/.dsh/.credentials.yaml（最高优先级）。当前已配置：${["exa", "tavily", "keenable", "firecrawl", "perplexity", "deepseek"].filter((k) => c[k]).map((k) => k.toUpperCase()).join(", ") || "无"}`,
      ...
      en: {
        ...
        keyStorageCredHint: (c) => `Saved to ~/.dsh/.credentials.yaml (highest priority). Currently configured: ${["exa", "tavily", "keenable", "firecrawl", "perplexity", "deepseek"].filter((k) => c[k]).map((k) => k.toUpperCase()).join(", ") || "none"}`,
          ...
          if (result.ok) {
            ...
            if (view) {
              ...
              try {
                ...
                if (cred.ok) {
                  const cc = {};
                  const map = { exaApiKey: "exa", tavilyApiKey: "tavily", keenableApiKey: "keenable", firecrawlApiKey: "firecrawl", perplexityApiKey: "perplexity", deepseekApiKey: "deepseek" };
