window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".dshweb-page{color:var(--dsw-alias-label-primary);max-width:1180px;margin:0 auto;padding:8px 18px 36px}",
      ".dshweb-hero{display:flex;justify-content:space-between;align-items:flex-start;gap:18px;margin:4px 0 22px}",
      ".dshweb-title{font-size:24px;line-height:1.25;margin:0 0 6px;font-weight:650}",
      ".dshweb-subtitle{color:var(--dsw-alias-label-secondary);font-size:13px;line-height:1.6;margin:0;max-width:720px}",
      ".dshweb-pill{border:1px solid rgba(80,200,120,.35);background:rgba(80,200,120,.12);color:#61c986;padding:5px 10px;border-radius:999px;font-size:12px;white-space:nowrap}",
      ".dshweb-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin-bottom:18px}",
      ".dshweb-provider{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:10px;padding:14px;min-width:0}",
      ".dshweb-providerTop{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px}",
      ".dshweb-providerName{font-size:14px;font-weight:650}",
      ".dshweb-providerMeta{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:1.5;min-height:36px}",
      ".dshweb-section{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:10px;padding:16px;margin-top:12px}",
      ".dshweb-sectionTitle{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px}",
      ".dshweb-sectionTitle h3{font-size:15px;margin:0}",
      ".dshweb-actions{display:flex;gap:8px;flex-wrap:wrap}",
      ".dshweb-textarea{width:100%;min-height:96px;resize:vertical;box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-specific-input-major);color:var(--dsw-alias-label-primary);border-radius:8px;padding:10px;font:12px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace}",
      ".dshweb-keyRows{display:grid;gap:6px;margin-top:12px}",
      ".dshweb-keyRow{display:grid;grid-template-columns:minmax(120px,1fr) 90px 120px 140px;gap:8px;align-items:center;border-top:1px solid var(--dsw-alias-border-l2);padding-top:7px;font-size:12px}",
      ".dshweb-muted{color:var(--dsw-alias-label-secondary);font-size:12px}",
      ".dshweb-status-ready{color:#61c986}.dshweb-status-invalid,.dshweb-status-exhausted{color:var(--dsw-alias-state-error-primary)}",
      "@media(max-width:720px){.dshweb-hero{flex-direction:column}.dshweb-keyRow{grid-template-columns:1fr 80px}.dshweb-keyRow>*:nth-child(n+3){display:none}}",
    ].join("");
    ...
    const PLUGIN_VERSION = "0.4.34-vyibc.2";
    const ENGINES = [
    ...
    async function bridgeProviderPool(path, payload) {
      const response = await fetch(`${BRIDGE_PREFIX}/provider-pool/${path}`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(payload ?? {}),
      });
    ...
    function WebSettingsPage() {
      ...
      const [drafts, setDrafts] = react.useState({ tavily: "", firecrawl: "" });
      const [busy, setBusy] = react.useState("");
      const [message, setMessage] = react.useState("");
      const load = react.useCallback(async () => {
        setBusy("load");
        const result = await bridgeProviderPool("status", {});
        if (result.ok) setPools(result.value);
        else setMessage(result.message || "Provider 状态读取失败");
        setBusy("");
      }, []);
      react.useEffect(() => { void load(); }, [load]);
      const save = async (provider) => {
        const keys = drafts[provider].split(/[\r\n,]+/).map((value) => value.trim()).filter(Boolean);
        if (!keys.length) { setMessage("请先粘贴至少一个 API Key"); return; }
        setBusy(`save:${provider}`);
        const result = await bridgeProviderPool("set", { provider, keys, mode: "append" });
        if (result.ok) {
          setDrafts((value) => ({ ...value, [provider]: "" }));
          setMessage(`${provider} 已安全写入金库，共 ${result.value.count} 个 Key`);
          await load();
      ...
      const refresh = async (provider) => {
        ...
        setMessage(provider === "tavily" ? "正在逐个验证 Tavily Key；官方未提供剩余额度接口。" : "正在刷新 Firecrawl 剩余额度。");
        const result = await bridgeProviderPool("refresh", { provider });
        if (result.ok) {
          setPools((value) => ({ ...(value || {}), [provider]: result.value }));
          setMessage(`${provider} 检测完成`);
        } else setMessage(result.message || "检测失败");
        setBusy("");
      };
      const providers = [
        { name: "DuckDuckGo", badge: "免费", meta: "无 Key。普通发现与导航优先。" },
        { name: "SearXNG", badge: "自托管", meta: "无商业 API 费用，成本来自自己的实例。" },
        { name: "Tavily", badge: `${pools?.tavily?.count ?? 0} Keys`, meta: "深度检索 Provider · 金库密钥池随机轮换。" },
        { name: "Firecrawl", badge: `${pools?.firecrawl?.count ?? 0} Keys`, meta: "复杂抓取与高质量检索 · 可读取官方额度。" },
        { name: "Bing", badge: "已退役", meta: "旧 Bing Search API 已退役，不再作为需 Key 的 Provider。" },
      ];
      const poolSection = (provider, title, applyUrl) => {
        const pool = pools?.[provider];
        return react_jsx_runtime.jsxs("section", { className: "dshweb-section", children: [
          react_jsx_runtime.jsxs("div", { className: "dshweb-sectionTitle", children: [
            react_jsx_runtime.jsxs("div", { children: [react_jsx_runtime.jsx("h3", { children: `${title} 密钥池` }), react_jsx_runtime.jsx("div", { className: "dshweb-muted", children: `随机选择 · ${pool?.enabledCount ?? 0}/${pool?.count ?? 0} 可用配置` })] }),
            react_jsx_runtime.jsxs("div", { className: "dshweb-actions", children: [
              react_jsx_runtime.jsx("a", { className: "dshfs-btn dshfs-link", href: applyUrl, target: "_blank", rel: "noreferrer", children: "申请 Key ↗" }),
              react_jsx_runtime.jsx("button", { className: "dshfs-btn", type: "button", disabled: !!busy || !pool?.count, onClick: () => refresh(provider), children: busy === `refresh:${provider}` ? "检测中…" : provider === "firecrawl" ? "刷新额度" : "验证全部" }),
            ] }),
          ] }),
          react_jsx_runtime.jsx("textarea", { className: "dshweb-textarea", value: drafts[provider], onChange: (event) => setDrafts((value) => ({ ...value, [provider]: event.target.value })), placeholder: "批量粘贴 API Key，每行一个。页面不会读取或回显已保存的密钥。" }),
          react_jsx_runtime.jsx("div", { className: "dshweb-actions", style: { marginTop: 8 }, children: react_jsx_runtime.jsx("button", { className: "dshfs-btn dshfs-save", type: "button", disabled: !!busy || !drafts[provider].trim(), onClick: () => save(provider), children: busy === `save:${provider}` ? "保存中…" : "追加到金库" }) }),
          react_jsx_runtime.jsx("div", { className: "dshweb-keyRows", children: (pool?.keys || []).map((entry) => react_jsx_runtime.jsxs("div", { className: "dshweb-keyRow", children: [
            react_jsx_runtime.jsx("code", { children: `#${entry.fingerprint}` }),
            react_jsx_runtime.jsx("span", { className: `dshweb-status-${entry.status}`, children: entry.status === "ready" ? "可用" : entry.status === "unchecked" ? "未检测" : entry.status }),
            react_jsx_runtime.jsx("span", { children: entry.remaining == null ? "额度未知" : `${entry.remaining}/${entry.limit ?? "—"}` }),
            react_jsx_runtime.jsx("span", { className: "dshweb-muted", children: entry.lastCheckedAt ? new Date(entry.lastCheckedAt).toLocaleString() : "尚未检测" }),
          ] }, entry.fingerprint)) }),
        ] });
      };
      return react_jsx_runtime.jsxs("div", { className: "dshweb-page", children: [
        react_jsx_runtime.jsxs("div", { className: "dshweb-hero", children: [
          react_jsx_runtime.jsxs("div", { children: [react_jsx_runtime.jsx("h2", { className: "dshweb-title", children: "Web" }), react_jsx_runtime.jsx("p", { className: "dshweb-subtitle", children: "配置 DSH 原生网页搜索的 Provider、密钥池与路由。它是 Plugin 能力，不会额外注册搜索 MCP。" })] }),
          react_jsx_runtime.jsx("span", { className: "dshweb-pill", children: "0 MCP 依赖" }),
        ] }),
        react_jsx_runtime.jsx("div", { className: "dshweb-grid", children: providers.map((provider) => react_jsx_runtime.jsxs("article", { className: "dshweb-provider", children: [react_jsx_runtime.jsxs("div", { className: "dshweb-providerTop", children: [react_jsx_runtime.jsx("span", { className: "dshweb-providerName", children: provider.name }), react_jsx_runtime.jsx("span", { className: "dshfs-badge", children: provider.badge })] }), react_jsx_runtime.jsx("div", { className: "dshweb-providerMeta", children: provider.meta })] }, provider.name)) }),
        busy === "load" && !pools ? react_jsx_runtime.jsx("div", { className: "dshweb-muted", children: "正在读取 Provider 状态…" }) : null,
        message ? react_jsx_runtime.jsx("div", { className: "dshweb-muted", style: { marginBottom: 8 }, children: message }) : null,
        poolSection("tavily", "Tavily", "https://app.tavily.com/home"),
        poolSection("firecrawl", "Firecrawl", "https://www.firecrawl.dev/app/api-keys"),
        react_jsx_runtime.jsxs("section", { className: "dshweb-section", children: [react_jsx_runtime.jsx("div", { className: "dshweb-sectionTitle", children: react_jsx_runtime.jsx("h3", { children: "路由与通用搜索设置" }) }), react_jsx_runtime.jsx(FreeSearchCard, { page: true })] }),
      ] });
    }
...
react_jsx_runtime.jsx("div", {
    ...
    function apply(ctx) {
      // 顶层 Web 设置：Provider、金库密钥池、检测与现有路由配置的统一入口。
