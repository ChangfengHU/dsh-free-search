...
const VYIBC_FIRECRAWL_ENGINE = "vyibc-firecrawl";
...
const ALL_ENGINES = [...KEYLESS_ENGINES, VYIBC_FIRECRAWL_ENGINE, ...PAID_ENGINES];
const TIME_ENGINES = ["tavily", "exa", "keenable", "firecrawl", VYIBC_FIRECRAWL_ENGINE, "parallel", "searxng", "ddg", "ddg-lite"];

// 当前插件版本（发布时与 package.json 同步）
const PLUGIN_VERSION = "0.4.33-vyibc.4";
...
let vyibcFirecrawlCallSeq = 0;

function firecrawlTimeFilter(timeRange) {
  if (!timeRange) return undefined;
  if (timeRange.after) return `cdr:1,cd_min:${formatFirecrawlDate(timeRange.after)}`;
  if (timeRange.days !== undefined) {
    const tr = approximateTimeRange(timeRange.days);
    return FIRECRAWL_TBS[tr];
  }
  return undefined;
}
...
function mcpText(result) {
  ...
  if (typeof result?.value?.content?.[0]?.text === "string") return result.value.content[0].text;
  return "";
}

async function callVyibcFirecrawlHttp(apiToken, args, signal) {
  if (!apiToken) throw new Error("vyibc-firecrawl credential is unavailable");
  const response = await fetch("https://fleet.vyibc.com/mcp/firecrawl", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      jsonrpc: "2.0",
      id: `free-search-vyibc-firecrawl-${Date.now()}-${++vyibcFirecrawlCallSeq}`,
      method: "tools/call",
      params: { name: "vyibc-firecrawl_search", arguments: args },
    }),
    signal: signal ?? AbortSignal.timeout(30000),
  });
  if (!response.ok) throw new Error(`vyibc-firecrawl MCP HTTP ${response.status}`);
  const rpc = await response.json();
  if (rpc.error) throw new Error(rpc.error.message || "vyibc-firecrawl MCP call failed");
  return rpc.result;
}

async function searchVyibcFirecrawl(tools, apiToken, query, maxResults, timeRange, signal) {
  const args = {
    query,
    limit: Math.min(Math.max(maxResults ?? 5, 1), 10),
    sources: ["web"],
  };
  const tbs = firecrawlTimeFilter(timeRange);
  if (tbs) args.tbs = tbs;
  let result;
  if (tools && typeof tools.execute === "function") {
    try {
      result = await tools.execute({
        callId: `free-search-vyibc-firecrawl-${Date.now()}-${++vyibcFirecrawlCallSeq}`,
        name: "mcp__vyibc-firecrawl__search",
        arguments: args,
        signal: signal ?? AbortSignal.timeout(30000),
      });
    } catch (cause) {
      // Older profiles may expose the credential but not register this MCP in ToolRuntime.
      // Use the same authenticated MCP endpoint so routing is not coupled to load order.
      if (!String(cause?.message ?? cause).includes("unknown tool")) throw cause;
    }
  }
  if (result?.isError && mcpText(result).includes("unknown tool")) result = undefined;
  if (!result) result = await callVyibcFirecrawlHttp(apiToken, args, signal);
  if (result?.isError) throw new Error(mcpText(result) || "vyibc-firecrawl MCP search failed");
  const text = mcpText(result);
  let envelope;
  try {
    envelope = JSON.parse(text);
  } catch {
    throw new Error("vyibc-firecrawl MCP returned invalid JSON");
  }
  if (envelope?.ok !== true) throw new Error(`vyibc-firecrawl MCP unavailable (${envelope?.code ?? "unknown"})`);
  ...
  const sources = rows
    ...
    .map((row) => ({
      ...
      ...(row.description || row.snippet || row.markdown
        ? { snippet: String(row.description ?? row.snippet ?? row.markdown).slice(0, 300) }
        : {}),
      ...(row.publishedDate || row.published_at
        ? { publishedAt: String(row.publishedDate ?? row.published_at) }
        : {}),
    }));
  return { sources: uniqueSources(sources, maxResults ?? 10), truncated: envelope.truncated === true };
}

function buildEngineChain(preferred, strategy, timeRange) {
  const ordered = strategy === "quality-first"
    ? [VYIBC_FIRECRAWL_ENGINE, preferred, ...KEYLESS_ENGINES]
    : [preferred, ...KEYLESS_ENGINES, VYIBC_FIRECRAWL_ENGINE];
...
function apply(ctx, config) {
  ...
  // vyibc-firecrawl 通过已注册的 MCP 工具调用，复用 MCP 客户端的认证、超时与审计。
  ...
  const provider = {
    ...
    async search(request, signal) {
      ...
      // 显式指定引擎仍优先；quality-first 未指定时直接从受控的 vyibc-firecrawl 开始。
      ...
      // free-first: 免费/匿名线路全部在前，最后才升级受控 Firecrawl。
      // quality-first: 直接使用 vyibc-firecrawl，失败后再回落免费线路。
      ...
      for (const engine of chain) {
        ...
        try {
          ...
          if (engine === "ddg") {
            ...
            const key = explicitEngine ? await resolveApiKey("TAVILY_API_KEY", "tavilyApiKey") : "";
          ...
          } else if (engine === VYIBC_FIRECRAWL_ENGINE) {
            const token = await resolveApiKey("FLEET_VAULT_TOKEN");
            result = await searchVyibcFirecrawl(getTools(), token, request.query, request.maxResults, timeRange, effSignal);
  ...
  const runEngineTest = async (engine, query, timeRange) => {
    ...
    const attempt = async () => {
      switch (engine) {
        ...
        case VYIBC_FIRECRAWL_ENGINE:
          return await searchVyibcFirecrawl(getTools(), q, 2, tr);
        case "parallel": {
      ...
      const dispose = sctx.tools.register(
        defineTool({
          ...
          parameters: {
            ...
            engine: {
              type: "string",
              description: "Optional specific engine to try first: ddg, ddg-lite, bing, searxng, anysearch, exa, tavily, keenable, vyibc-firecrawl, firecrawl, parallel, perplexity, serpbase, deepseek-official.",
            },
            strategy: {
              type: "string",
              enum: ["free-first", "quality-first"],
              description: "Routing strategy. free-first tries anonymous/free engines before vyibc-firecrawl. quality-first starts with vyibc-firecrawl and falls back to free engines.",
            },
            minimumSources: {
              type: "number",
    ...
    refreshPrompt = () => {
      ...
      disposeSection = sctx.systemPrompt.section({
        ...
        text: [
          ...
          "- vyibc-firecrawl - managed Firecrawl MCP; credentials remain in Fleet Vault",
