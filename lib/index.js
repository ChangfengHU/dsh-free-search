...
const ALL_ENGINES = ["ddg", "ddg-lite", "bing", "searxng", "anysearch", "exa", "tavily", "keenable", "firecrawl", "perplexity", "deepseek-official"];
...
function apply(ctx, config) {
  ...
  const provider = {
    ...
    async search(request, signal) {
      ...
      const paidEngines = ["exa", "tavily", "keenable", "firecrawl", "perplexity", "deepseek-official"];
      const freeEngines = ["bing", "anysearch", "ddg", "ddg-lite", "searxng"];
      // 支持 time_range 过滤的引擎：tavily / exa / keenable / firecrawl / searxng / ddg / ddg-lite
      const timeEngines = ["tavily", "exa", "keenable", "firecrawl", "searxng", "ddg", "ddg-lite"];
      let chain;
      ...
      const dispose = sctx.tools.register(
        defineTool({
          ...
          parameters: {
            engines: {
              type: "array",
              description: "Which engines to test (default: all). Options: ddg, ddg-lite, bing, searxng, anysearch, exa, tavily, keenable, firecrawl, perplexity, deepseek-official.",
              items: { type: "string" },
            },
            query: {
              type: "string",
      ...
      const dispose = sctx.tools.register(
        defineTool({
          ...
          parameters: {
            ...
            engine: {
              type: "string",
              description: "Optional specific engine to try first: ddg, ddg-lite, bing, searxng, anysearch, exa, tavily, keenable, firecrawl, perplexity, deepseek-official.",
            },
          },
          output: {
            schema: {
